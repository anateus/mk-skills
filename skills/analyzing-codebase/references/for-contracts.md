...
The current helper uses the TypeScript compiler and Python's standard-library AST. It discovers bounded declaration, schema, route, registration, and HTTP client patterns without importing target modules or starting services. Inspect the reported adapter capabilities and gaps before using an extracted fact. Native Pydantic/FastAPI or ts-rest/Zod runtime exports, generated language models, DataHub publication, AsyncAPI export, and recorded-traffic extraction are follow-on capabilities, not commands provided by this helper.
