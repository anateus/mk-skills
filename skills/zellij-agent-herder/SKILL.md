...
- **Leave review notes as work lands.** Each `--watch` pane is a live hunk session. Before reviewing one, run `hunk skill path` and read the returned skill completely so the review uses the installed Hunk version's current guidance. Whenever a significant chunk of work completes (a worktree finishes, or the aggregate lands), use that **`hunk-review`** workflow to walk the hunks and drop inline comments on the diff — drive it through `hunk session *`, never a hand-run `hunk diff`. The notes stay attached to the live session for a reviewer or the human to read.
...
- **Review titles preserve lineage compactly.** A Hunk pane inherits its originating agent's emoji ancestry and appends the review leaf, for example `🦀 ▸ 🌿 ▸ 🔍`. If identity metadata is unavailable, it renders as `🔍`.
