...
const checks = [
  ...
  ['Curation tests', 'python3', ['-m', 'unittest', 'tests.curation.test_review_upstreams', '-v']],
];
...
const systemDependenciesAvailable = configuredValidator || spawnSync(
...
if (fs.existsSync(pluginValidator) && systemDependenciesAvailable) {
  ...
  process.stdout.write('\n==> Plugin schema\nSKIP plugin-schema check: validator unavailable; set PLUGIN_VALIDATOR to enable it.\n');
}
