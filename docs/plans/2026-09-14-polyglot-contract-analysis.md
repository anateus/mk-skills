...
This plan defines the staged target. The first static helper and the contract/testing/transition workflows are now implemented; the remaining native export, conversion, external API expansion, and DataHub work stays proposed. See the [first-slice validation record](../validation/contract-inventory-first-slice.md) for tested scope and limits. This plan supersedes the TypeScript-first sequence in the [tooling investigation](../designs/2026-09-14-contract-inventory-tooling.md), which contains the earlier executable probes and tool qualifications.
