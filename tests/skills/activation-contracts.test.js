...
const descriptions = {
  'clarifying-work': 'Reduce consequential uncertainty by inspecting facts, separating assumptions from decisions, and mapping integration risks. Use when a request is ambiguous, integration-heavy, cross-boundary, or likely to hide decisions that could cause substantial rework. Consider whenever drafting or filing a tracker ticket (e.g. Linear), so the ticket records decisions rather than assumptions.',
  ...
  'specifying-work': 'Turn agreed intent and repository evidence into a concise behavior contract with acceptance criteria, testing seams, constraints, and non-goals. Use when the user asks for a specification, design contract, requirements, or acceptance criteria before planning or implementation. Also use when drafting a tracker ticket that is more than a small placeholder: the ticket body is the behavior contract.',
